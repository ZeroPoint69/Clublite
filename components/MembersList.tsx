    ...
    <div className="max-w-xl mx-auto pb-20 pt-4 px-2">
      <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-200 mb-4">
        <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
          Club Members
          <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full font-medium">{members.length} Total</span>
        </h2>
        <div className="relative">
          ...
          <input 
            ...
            className="w-full bg-gray-50 rounded-lg pl-10 pr-4 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary/20"
          />
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        {filteredMembers.length > 0 ? (
              ...
              <div key={member.id} className="p-4 flex items-center justify-between hover:bg-gray-50 active:bg-gray-100/50 transition-all">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className={`text-[10px] uppercase font-black tracking-widest ${member.role === 'admin' ? 'text-primary' : 'text-gray-400'}`}>
                      {member.role}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <>
                      <button 
                        ...
                        className={`p-2 rounded-full transition-all active:scale-90 ${
                          member.role === 'admin' ? 'text-primary bg-primary/5 hover:bg-primary/10' : 'text-gray-400 hover:bg-gray-100'
                        }`}
                  ...
                  {!isAdmin && member.id !== currentUser?.id && (
                    <button className="bg-gray-50 hover:bg-gray-100 active:scale-90 p-2 rounded-full text-gray-400 transition-all border border-gray-100">
                      <UserPlus size={18} />
                    </button>
                  )}
                </div>
          ...
          <div className="p-10 text-center text-gray-400">No members found matching "{search}"</div>
        )}
      </div>
