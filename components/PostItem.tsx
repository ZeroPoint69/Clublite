...
const PostItem: React.FC<PostItemProps> = ({ post, currentUser }) => {
  ...
  return (
    <div className="bg-surface shadow-lg mb-4 border-b border-border md:rounded-xl md:border overflow-hidden text-text">
      <div className="p-4 flex justify-between items-start">
        ...
        <div className="relative" ref={menuRef}>
          <button 
            ...
            className="text-text-secondary hover:bg-bg active:scale-90 p-2 rounded-full transition-all"
            title="More options"
          >
          ...
          {showMenu && (
            <div className="absolute right-0 mt-1 w-48 bg-surface border border-border rounded-lg shadow-2xl py-1 z-20 animate-in fade-in zoom-in-95 duration-100">
              {canDeletePost ? (
                <button 
                  onClick={() => { setShowMenu(false); setConfirmDeletePost(true); }}
                  className="w-full text-left px-4 py-2 text-sm text-red-400 hover:bg-red-900/20 active:bg-red-900/30 flex items-center gap-2 font-medium transition-colors"
                >
      ...
      {post.image && (
         <div className="mt-2 bg-bg flex items-center justify-center">
            <img src={post.image} alt="Post content" className="w-full h-auto max-h-[500px] object-contain" />
         </div>
      )}

      <div className="px-4 py-2 flex justify-between text-text-secondary text-sm border-b border-border">
        <div className="flex items-center gap-1">
          {post.likes.length > 0 && (
      ...
      <div className="px-2 py-1 flex justify-between items-center">
        <button 
          onClick={handleLike}
          className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg font-medium text-sm transition-all active:scale-95 ${
            isLiked ? 'text-primary' : 'text-text-secondary hover:bg-bg'
          }`}
        >
        ...
        <button 
          onClick={() => setShowComments(!showComments)}
          className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg font-medium text-sm text-text-secondary hover:bg-bg active:scale-95 transition-all"
        >
            ...
            {loadingComments ? (
                ...
                <div key={comment.id} className="flex gap-2 group">
                    ...
                    <div className="flex-1">
                        <div className="bg-bg rounded-2xl px-3 py-2 inline-block max-w-[90%]">
                            <div className="flex items-center gap-1">
                            ...
                            <span className="text-sm text-text break-words">{comment.content}</span>
                        </div>
                        <div className="flex gap-4 text-xs text-text-secondary ml-3 mt-1">
                            ...
                            <button 
                                ...
                                className="font-medium hover:text-red-400 hover:underline active:opacity-50 transition-all"
                            >
                                Delete
                            </button>
                            )}
          ...
          <form onSubmit={handleSubmitComment} className="p-4 pt-2 flex gap-2 items-center">
             ...
             <div className="flex-1 relative">
                <input
                    ...
                    className="w-full bg-bg border border-border rounded-full pl-4 pr-10 py-2.5 text-[16px] focus:outline-none focus:border-primary text-text"
                    disabled={sendingComment}
                />
                <button 
                  type="submit"
                  disabled={!newComment.trim() || sendingComment}
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-primary disabled:text-text-secondary p-1.5 hover:bg-surface active:scale-90 rounded-full transition-all"
                >
